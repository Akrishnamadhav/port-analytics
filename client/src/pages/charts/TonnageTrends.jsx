...
const TonnageTrends = () => {
  ...
  const yAxisFormatter = (v) => (v >= 1000 ? `${(v / 1000).toFixed(0)}K` : v);

  const tooltipFormatter = (value, name) => {
    const formatted = value?.toLocaleString('en-IN') || '0';
    return [`${formatted} MT`, 'Tonnage'];
  };
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-port-border p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-port-text">Tonnage Trends</h2>
            <p className="text-sm text-port-muted">
              {mode === 'yearly' ? 'Year-over-year tonnage' : `Monthly tonnage for ${selectedYear}`}
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-80 text-port-muted">
          ...
          <p className="text-sm">No tonnage data available</p>
        </div>
      ) : (
