...
const VesselRevenueBreakdown = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get(`/api/stats/vessel-revenue-breakdown?year=${selectedYear}`);
      setData(res.data || []);
    } catch (err) {
      console.error('Failed to fetch vessel revenue breakdown:', err);
      setData([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-port-border p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-port-text">Vessel Revenue Breakdown</h2>
            <p className="text-sm text-port-muted">Vessel-wise distribution of generated revenue</p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-80 text-port-muted">
          ...
          <p className="text-sm">No vessel revenue data available for {selectedYear}</p>
        </div>
      ) : (
...
export default VesselRevenueBreakdown;
